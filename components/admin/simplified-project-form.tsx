...
export function SimplifiedProjectForm({
  ...
  const [formData, setFormData] = useState({
    ...
    key_features: initialData?.key_features
      ? typeof initialData.key_features === "string"
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (result.success && Array.isArray(result.data)) {
          const formatted: ProjectCategoryOption[] = result.data.map(
            (category: any) => ({
              value: category.id,
              label: category.name,
            })
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      if (
        prev.category &&
        categories.some((cat) => cat.value === prev.category)
      ) {
        return prev;
      }
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                handleInputChange("project_info_details", e.target.value)
              }
          ...
          <div className="space-y-2">
            <Label htmlFor="key_features" className="text-sm font-medium">
            ...
            <p className="text-xs text-muted-foreground">
              Separate multiple features with commas. These will be displayed as
